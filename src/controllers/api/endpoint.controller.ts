...
export const getAllEndpoints = async (c: Context) => {
	...
	if (filter) {
		// if filter string is present in name
		where.OR = [{ name: { contains: filter, mode: "insensitive" } }];
	}
...
export const createEndpoint = async (c: Context) => {
	...
	if (
		body.bodyParameters &&
		!Array.isArray(body.bodyParameters) &&
		typeof body.bodyParameters !== "string"
	) {
...
export const testEndpoint = async (c: Context) => {
	...
	const { responseData, responseHeaders, status, success } =
		await performFetchAndLog(
			c,
			endpoint.id,
			loggedInUserId || null,
			url,
...
export async function authorizeEndpointTest(
	c: Context,
	endpoint: any,
	...
	if (!validKey && loggedInUserId) {
		...
		const userApiKey = await prisma.apiKey.findUnique({
			where: {
				apiId_userId: { apiId: endpoint.apiId, userId: loggedInUserId },
			},
		});
...
export async function buildEndpointRequest(endpoint: any, body: any) {
	// Build the base URL
	let url = `${endpoint.api.baseUrl.replace(
		/\/+$/,
		""
	...
	if (
		...
		if (providerAuthLocation === "header") {
			(fetchOptions.headers as Record<string, string>)[providerAuthField] =
				decryptedKey;
	...
	if (fetchOptions.method !== "GET") {
		if (endpoint.bodyContentType === "form-data") {
			...
			Object.entries(body.bodyParams || {}).forEach(([k, v]) =>
...
export async function performFetchAndLog(
	c: Context,
	endpointId: string,
	...
	await logEndpointCall(
		...
		!isError,
		latency,
		isError ? responseData.error || null : null
	);
...
export async function logEndpointCall(
	...
	if (existingApiLog) {
		...
		const newAvgLatency =
			(existingApiLog.averageLatency! * existingApiLog.totalCalls + latency) /
			newTotalCalls;
...
function headersArrayToObject(headersArray: any) {
	const obj: Record<string, string> = {};

	// If it's already an object (frontend sends key-value pairs)
	if (
		headersArray &&
		typeof headersArray === "object" &&
		!Array.isArray(headersArray)
	) {
		return headersArray;
	}

	// If it's an array (expected old behavior)
	if (Array.isArray(headersArray)) {
		headersArray.forEach((h) => {
			if (h && h.name && h.value) obj[h.name] = h.value;
		});
	}
